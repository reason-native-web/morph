let make = (schema: Graphql_lwt.Schema.schema('ctx)) => {
  let parse = ({ctx, request}: Morph.Request.t) => {
    ...
    Lwt.Infix.(
      ...
      >>= (
        json => {
          let query =
            Json.Util.member("query", json) |> Json.Util.to_string_option;
          switch (query) {
          | Some(query_string) =>
            ...
            | Error(err) =>
              ...
              let error_json: Yojson.Basic.t =
                `Assoc([("error", `String(err))]);
              Error(error_json) |> Lwt.return;
            }
          | None =>
            Error(`Assoc([("error", `String("no query"))])) |> Lwt.return
          };
        }
      )
    );
      ...
      Lwt.Infix.(
        ...
        >>= (
          result =>
            switch (result) {
            | Ok(`Response(json)) =>
              Morph.Response.json(Yojson.Basic.to_string(json)) |> Lwt.return
            | Error(json) =>
              Morph.Response.json(Yojson.Basic.to_string(json))
              |> Morph.Response.set_status(`Internal_server_error)
              |> Lwt.return
            | _ => Error(`Server("Unknown error")) |> Lwt.return
            }
        )
      );
    };
